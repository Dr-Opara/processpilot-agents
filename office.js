...
function stationData(s){
  if(s.type==='region'){
    const r=regionData(s.code);return {id:`region-${s.code}`,title:r?.employee||`${s.code} Procurement Agent`,subtitle:r?.name||s.code,state:regionalConnected()?'live_idle':'blocked',color:s.color,region:r};
  }
  const spec=VISIBLE_SPECIALISTS.find(v=>v.key===s.key),a=agent(spec.agentId);return {id:a?.id||spec.agentId,title:spec.label,subtitle:a?.department||'Specialist',state:agentState(spec.agentId),color:spec.color,agent:a};
}
function stationMarkup(s,i){const d=stationData(s),status=stateLabel(d.state);return `<button class="live-station ${d.state}" style="left:${s.x}%;top:${s.y}%;--scale:${s.scale};--agent:${d.color};--delay:${(i%5)*-.55}s" data-kind="${s.type}" data-key="${s.type==='region'?s.code:s.key}" aria-label="${esc(d.title)} — ${status}">
<span class="desk"><i class="monitor"><i class="scan"></i><b>${status.toUpperCase()}</b><small>${String(i+1).padStart(2,'0')}</small></i></span>
<span class="robot"><i class="antenna"></i><span class="bot-head"><i class="visor"><i></i><i></i></i></span><span class="bot-body"></span><span class="bot-arm left"></span><span class="bot-arm right"></span></span>
...
function renderStatus(){
  const p=runtime?.progress||{},apps=runtime?.approvals||[],events=runtime?.events||[],s=specialistState();
  const visibleStates=STATIONS.map(st=>stationData(st).state),visibleReady=visibleStates.filter(v=>v!=='blocked').length;
  $('#agentCount').textContent=registry.agents.length;$('#regionalReady').textContent=`${visibleReady} / 10`;$('#systemState').textContent=visibleReady===10?'All Visible Agents Online':'Attention Required';$('#backgroundReady').textContent=Math.max(0,(runtime?.readyAgents||0)-6);$('#blockedCount').textContent=s.blocked;$('#queueDepth').textContent=runtime?.queueDepth||0;$('#availability').textContent=runtime?'ONLINE':'ERROR';$('#reviewBadge').textContent=`${apps.length} decision${apps.length===1?'':'s'}`;
  $('#regionalMini').innerHTML=STATIONS.slice(0,10).map(st=>{const d=stationData(st);return `<span><i></i><b>${st.type==='region'?st.code:VISIBLE_SPECIALISTS.find(v=>v.key===st.key)?.label.replace(' Agent','')}</b><small>${stateLabel(d.state)}</small></span>`}).join('');
  ...
  $('#approvals').innerHTML=apps.length?apps.slice(0,3).map(x=>`<div><b>${esc(x.title)}</b><p>${esc(x.summary||'Executive decision required')}</p></div>`).join(''):'<div><b>No executive decisions waiting</b><p>Decision packets appear here when work reaches a governance gate.</p></div>';
  ...
  $('#specialistSummary').innerHTML=DEPTS.map(d=>{const members=registry.agents.filter(a=>a.department===d),working=members.filter(a=>agentState(a.id)==='working').length;return `<button class="specialist-dept" data-dept="${esc(d)}"><b>${esc(d)}</b><small>${working?working+' working now':members.length+' ready in background'}</small></button>`}).join('');
...
function showWorkspace(view){const p=runtime?.progress||{},apps=runtime?.approvals||[],events=runtime?.events||[];const renderers={
'regional-agents':()=>`<small>LIVE OFFICE</small><h2>Visible Operations Agents</h2><p>10 visible stations: TX, DC, MD, VA, Research, Outreach, Compliance, Pricing, Contracts and Proposal.</p>`,
opportunities:()=>`<small>PIPELINE</small><h2>Opportunities</h2><p><b>${p.found||0}</b> opportunities are currently recorded in the live pipeline.</p><div class="detail-section"><h3>QUALIFICATION</h3><p>${p.qualified||0} currently meet the qualification threshold.</p></div>`,
'work-queue':()=>`<small>OPERATIONS</small><h2>Work Queue</h2><p><b>${runtime?.queueDepth||0}</b> work packets are queued, running or waiting at governed checkpoints.</p><div class="detail-section"><h3>EXECUTION</h3><p>${runtime?.verifiedAgentExecutions||0} verified specialist executions are recorded.</p></div>`,
proposals:()=>`<small>PROPOSAL STUDIO</small><h2>Proposals</h2><p><b>${p.proposals||0}</b> proposals are currently in progress.</p>`,
outreach:()=>`<small>BUSINESS DEVELOPMENT</small><h2>Outreach</h2><p>Outreach is prepared by the visible Outreach Agent and supporting specialists. External sends remain governed.</p>`,
contracts:()=>`<small>CONTRACTS & DELIVERY</small><h2>Contracts</h2><p>Contracts Agent reviews award and contract materials while delivery specialists operate behind the scenes.</p>`,
reports:()=>`<small>ANALYTICS</small><h2>Reports & Analytics</h2><p><b>${events.length}</b> recent verified runtime events are visible. ${apps.length} executive decision packet${apps.length===1?' is':'s are'} pending.</p>`,
