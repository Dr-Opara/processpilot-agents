...
function renderStatus(){
  const p=runtime?.progress||{},pendingCount=runtime?.pendingApprovalCount||0,recentCount=runtime?.recentEventCount||0,s=specialistState();
  const visibleStates=STATIONS.map(st=>stationData(st).state),visibleReady=visibleStates.filter(v=>v!=='blocked').length;
  $('#agentCount').textContent=registry.agents.length;
  $('#regionalReady').textContent=`${visibleReady} / 10`;
  $('#systemState').textContent=visibleReady===10?'All Visible Agents Online':'Attention Required';
  $('#backgroundReady').textContent=Math.max(0,(runtime?.readyAgents||0)-6);
  $('#blockedCount').textContent=s.blocked;
  $('#queueDepth').textContent=runtime?.queueDepth||0;
  $('#availability').textContent=runtime?'ONLINE':'ERROR';
  $('#reviewBadge').textContent=`${pendingCount} decision${pendingCount===1?'':'s'}`;
  // Global header badge -- visible regardless of which route is active.
  const decisionsCount=$('#decisionsCount');
  if(decisionsCount){decisionsCount.textContent=String(pendingCount);document.getElementById('decisionsTrigger')?.classList.toggle('has-pending',pendingCount>0)}
  ...
  $('#progress').innerHTML=[['Opportunities Found',p.found||0],['Qualified',p.qualified||0],['Proposals in Progress',p.proposals||0],['Submitted',p.submitted||0],['Approvals Pending',pendingCount]].map(x=>`<li>${x[0]} <b>${x[1]}</b></li>`).join('');
  // Public Live Office view intentionally shows COUNTS ONLY here -- approval
  // titles/summaries can contain real business detail and require signing
  // in to Executive Command to review (see api/status.js for why).
  $('#approvals').innerHTML=pendingCount
    ?`<div><b>${pendingCount} decision${pendingCount===1?'':'s'} awaiting executive review</b><p>Sign in to Executive Command to see details and decide.</p></div>`
    :'<div><b>No executive decisions waiting</b><p>Decision packets appear here when work reaches a governance gate.</p></div>';
  $('#activity').innerHTML=recentCount
    ?`<div><time>NOW</time><span>${recentCount} verified event${recentCount===1?'':'s'} recorded recently.</span><b class="verified">VERIFIED</b></div><div><time></time><span>Sign in for the full activity log.</span><b></b></div>`
    :'<div><time>NOW</time><span>No verified execution events yet.</span><b class="waiting">IDLE</b></div>';
  ...
  document.querySelectorAll('.specialist-dept').forEach(b=>b.onclick=()=>showDepartment(b.dataset.dept));
  document.body.classList.toggle('has-approval',pendingCount>0);
}
...
export function initLiveOffice(){
  $('#closeDetail').onclick=()=>$('#detail').close();
  $('#chief').onclick=()=>openExecutiveDrawer();
  $('#openQueue').onclick=()=>openExecutiveDrawer();
