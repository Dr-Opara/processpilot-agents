...
async function setSourceResult(source,{success,health,error=null,records=0,mode='direct_public'}){
  health=health||(success?'healthy':'failed');
  const now=new Date().toISOString();
  const patch={last_checked_at:now,last_error:error,config:{collector_mode:mode,last_records:records,last_status:health}};
  // Only set last_success_at on an actual success -- never clobber the
  // historical record of the last successful collection with null just
  // because *this* check failed or was degraded. Omitting the key from the
  // PATCH body leaves the existing column value untouched.
  if(success)patch.last_success_at=now;
  await db(`source_registry?code=eq.${encodeURIComponent(source.code)}`,{method:'PATCH',body:JSON.stringify(patch)}).catch(()=>{});
}
...
export async function collectPortal(source,{maxDetailPages=12}={}){
  ...
  // Three-state health, not a bare success/fail boolean: root-page-reachable
  // is not the same as successful retrieval, and a run where some candidate
  // pages succeeded and others errored is neither fully healthy nor a total
  // failure -- it must be visible as "degraded", not silently reported healthy.
  let health,success;
  if(result.candidates===0){health='healthy';success=true;}
  else if(result.retrieved===0){health='failed';success=false;}
  else if(result.errors.length>0){health='degraded';success=true;}
  else{health='healthy';success=true;}
  const error=health==='failed'?(result.errors[0]||'No candidate pages could be retrieved successfully'):(result.errors[0]||null);
  await setSourceResult(source,{success,health,records:result.ingested,error});
  return result;
}
