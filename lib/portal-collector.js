...
export async function collectPortal(source,{maxDetailPages=12}={}){
  const result={source:source.code,reachable:false,candidates:0,ingested:0,duplicates:0,excluded:0,errors:[]};
  let root;
  ...
  for(const candidate of unique){
    try{
      const page=await fetchText(candidate.url);if(!page.ok)continue;
  ...
  await setSourceResult(source,{success:true,records:result.ingested,error:result.errors[0]||null});
  return result;
}
